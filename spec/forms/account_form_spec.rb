...
RSpec.describe AccountForm, :type => :model do
  ...
  it "並び替えがfollower_increment,post_increment,follower_increment_rate以外であれば無効である" do
    account_form = AccountForm.new(sort: "hoge_sort")
    expect(account_form.valid?).to eq false

    valid_sorts = %w(follower_increment post_increment follower_increment_rate)
